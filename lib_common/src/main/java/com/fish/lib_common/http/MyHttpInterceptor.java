...
public class MyHttpInterceptor implements Interceptor {
    ...
    private final MyHttpInterceptor.Logger logger;
    private volatile MyHttpInterceptor.Level level;

    public MyHttpInterceptor() {
        this(MyHttpInterceptor.Logger.DEFAULT);
    }

    private MyHttpInterceptor(MyHttpInterceptor.Logger logger) {
        this.level = Level.BODY;
        this.logger = logger;
    }

    public Response intercept(Chain chain) throws IOException {
        MyHttpInterceptor.Level level = this.level;
        Request request = chain.request();
        if (level == MyHttpInterceptor.Level.NONE || !BuildConfig.DEBUG) {
            return chain.proceed(request);
        } else {
            boolean logBody = level == MyHttpInterceptor.Level.BODY;
            boolean logHeaders = logBody || level == MyHttpInterceptor.Level.HEADERS;
            ...
            try {
                ...
                this.logger.log("<-- FAILED " + request.url().toString() + "   " + var27);
                throw var27;
            }
    ...
    public interface Logger {
        MyHttpInterceptor.Logger DEFAULT = new MyHttpInterceptor.Logger() {
